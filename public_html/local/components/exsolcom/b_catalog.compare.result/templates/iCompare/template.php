	...
	<div class="swiper-container ilab_c_products j_products i_cs_tile"><div class="swiper-wrapper">
			<!-- Product -->
				...
				<?else*/if( $e['MIN_PRICE'] && $e['MIN_PRICE']['CAN_ACCESS'] ):// Цена [MIN_PRICE]
										// массивы вида [0]=>88 990,00 [1]=>тг.
										$min = preg_split('/\s(?=[^0-9])/', $e['I_CURRENCY_MIN_OFFER_PRICE_FORMATTED']);// MIN_PRICE

										if ( $e['MIN_PRICE']['DISCOUNT_VALUE'] < $e['MIN_PRICE']['VALUE'] ):// Скидка
											$dis = preg_split('/\s(?=[^0-9])/', $e['MIN_PRICE']['PRINT_VALUE']);?>
											<s class="i_pr_disc">
												<span class="i_price"><?=$dis[0]?></span>&nbsp;<span class="i_currency"><?=$dis[1]?></span>
											</s>
										<?elseif( $pr_max = i_price($e['PRICES'], $e['MIN_PRICE']['DISCOUNT_VALUE']) ):// или вычислим самую наибольшую цену
											$max = preg_split('/\s(?=[^0-9])/', $e['PRICES'][$pr_max]['PRINT_VALUE']);?>
											<s class="i_pr_disc">
												<span class="i_price"><?=$max[0]?></span>&nbsp;<span class="i_currency"><?=$max[1]?></span>
											</s>
										<?endif?>

										<span class="i_pr_min<?//if( strlen($e['MIN_PRICE']['DISCOUNT_VALUE'])>9 )echo ' ifont120'?>">
						<span class="i_price"><?=$min[0]?></span>&nbsp;<span class="i_currency"><?=$min[1]?></span>
									...
									<?elseif( $e['PRINT_MIN_OFFER_PRICE'] ):
										...
										<span class="i_pr_off">
						<span class="i_pr_from"><?=GetMessage('FROM')?></span>&nbsp;<span class="i_price<?//if( strlen($e['MIN_OFFER_PRICE'])>7 )echo ' ifont130'?>"><?=$off[0]?></span>&nbsp;<span class="i_currency"><?=$off[1]?></span>
					</span>
									<?else:
							...
							<div class="ilab_c_i_buy i_item_buy">
								...
								<?// -------------------------------------------------- Купить ?>
								<?if( $e['I_TRADE_OFFERS'] ):?>
									<a href="<?=$e['DETAIL_PAGE_URL']?>" class="i_buy_button i_sec_to_order"><?=GetMessage('READ_MORE')?></a>
								<?elseif($e['CAN_BUY']):?>

									<a href="<?=$e['BUY_URL']?>" class="i_buy_button jq_buy" jq_id="<?=$e['ID']?>" jqcount="1" price="<?if($e['MIN_PRICE']['DISCOUNT_VALUE'])echo $e['MIN_PRICE']['DISCOUNT_VALUE']?>" jqcount="<?=$e['CATALOG_MEASURE_RATIO']?>"<?if( $e['PROPERTIES']['I_MULTI_PRICE']['VALUE']=='Y' )echo ' style="display:none"'?>><?=GetMessage('BUY')?></a>
									...
									<div class="i_item_remove jq_item_remove idn" data-id="<?//=$e['ID']?>"></div>
									...
									<?if( $e['PROPERTIES']['I_MULTI_PRICE']['VALUE']=='Y' ):?>
										...
										<?if( $arParams['I_SHOW_NUMBER']=='Y' ):?>
											<div class="i_mbuy jq_mbuy jq_buy_count idn">
												...
												<div class="i_msaving">
													<span class="i_mstxt"><?=GetMessage('M_SAVING')?></span> <span class="i_msnumb jq_msnumb">-<?=$e['MIN_PRICE']['PRINT_DISCOUNT_DIFF']?></span>
												</div>
												...
												<div class="i_item_buy">
													...
													<?if( $e['I_TRADE_OFFERS'] ):?>
														<a href="<?=$e['DETAIL_PAGE_URL']?>" class="i_buy_button i_sec_to_order"><?=GetMessage('READ_MORE')?></a>
													<?elseif($e['CAN_BUY']):?>
														<a href="<?=$e['BUY_URL']?>" class="i_buy_button jq_buy" jq_id="<?=$e['ID']?>" jqcount="1" price="<?if($e['MIN_PRICE']['DISCOUNT_VALUE'])echo $e['MIN_PRICE']['DISCOUNT_VALUE']?>" jqcount="<?=$e['CATALOG_MEASURE_RATIO']?>"><?=GetMessage('BUY')?></a>
														<a href="<?=SITE_DIR?>personal/basket.php" class="i_buy_bought jq_bought idn">
															<span class="i_text"><?=GetMessage('IN_BASKET')?>:</span>&nbsp;
															<span class="i_m_ratio j_m_ratio"><?=$e['CATALOG_MEASURE_RATIO']?></span>&nbsp;
															<span class="i_m_name j_m_name"><?=$e['CATALOG_MEASURE_NAME']?></span>
...
<?endif*/?>
						...
						<a class="ilab_c_i_name" href="<?=$e['DETAIL_PAGE_URL']?>"><? echo ($e['PROPERTIES']['I_NAME_'.strtoupper(LANGUAGE_ID)]['VALUE']) ? $e['PROPERTIES']['I_NAME_'.strtoupper(LANGUAGE_ID)]['VALUE'] : $e['NAME']?></a>
					</div>
				<?endforeach?>
			<!-- Product -->
			...
			<!-- Property -->
				<div class="swiper-container ilab_c_property j_property"><div class="swiper-wrapper">
					<?if($arResult['I_PRO']):
						...
						<div class="swiper-slide"><div class="ilab_c_prop_empty"><?=Loc::getMessage('COMP_CHARACTERISTICS_NO',true,$l)?></div></div>
					<?endif?>
				</div></div>
			<!-- Property -->
	</div></div>
